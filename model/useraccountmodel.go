...
type (
	UserAccountModel interface {
		...
		TxAdjustTrading(tx *sql.Tx, userId int64, amount int64) (sql.Result, error)
	...
	UserAccount struct {
		...
		TradingBalance float64   `db:"trading_balance"`
...
func (m *defaultUserAccountModel) TxAdjustTrading(tx *sql.Tx, userId int64, amount int64) (sql.Result, error) {
	IdKey := fmt.Sprintf("%s%v", cacheUserAccountUserIdPrefix, userId)
	return m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("update %s set trading_balance=trading_balance+? where user_id=? and trading_balance + ? + balance >= 0", m.table)
		return tx.Exec(query, amount, userId, amount)
	}, IdKey)
}
...
func (m *defaultUserAccountModel) TxAdjustBalance(tx *sql.Tx, userId int64, amount int64) (sql.Result, error) {
	IdKey := fmt.Sprintf("%s%v", cacheUserAccountUserIdPrefix, userId)
	return m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("update %s set trading_balance=trading_balance-?, balance=balance+? where user_id=?", m.table)
...
func (m *defaultUserAccountModel) Insert(data *UserAccount) (sql.Result, error) {
	...
	ret, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.Exec(query, data.UserId, data.Balance, data.TradingBalance)
...
func (m *defaultUserAccountModel) Update(data *UserAccount) error {
	...
	_, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.Exec(query, data.UserId, data.Balance, data.TradingBalance, data.Id)
	}, userAccountIdKey, userAccountUserIdKey)
	return err
}
